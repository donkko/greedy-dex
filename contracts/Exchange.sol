...
contract Exchange is owned {
    ...
    function addToken(string symbolName, address erc20TokenAddress, uint ExtDeposit) public  {
        require(tokenBalanceForAddress[msg.sender][1] > ExtDeposit);
        require(ExtDeposit > 0 );
        require(!hasToken(symbolName));
        require(tokenIndex + 1 > tokenIndex);
        tokenIndex++;

        //actually subtract the amount of tokens to change it then
        tokenBalanceForAddress[msg.sender][1] -= ExtDeposit;
        tokens[tokenIndex].symbolName = symbolName;
        tokens[tokenIndex].tokenContract = erc20TokenAddress;
        tokens[tokenIndex].ExtDeposit = ExtDeposit;
